...
class BusinessUnderstandingEngine:
    ...
    def analyze(self, goal: str, scenario: dict | None = None):
        ...
        dna = {
            ...
            "repeat_purchase_potential": self._score_repeat_purchase(text, business_model),
            "capital_intensity": self._score_capital_intensity(text, business_model, channel),
            "operational_complexity": self._score_operational_complexity(text, business_model),
        ...
        return {
            ...
            "strategic_direction": self._strategic_direction(dna),
        }
    ...
    def _extract_budget(self, text: str):
        ...
        if any(x in text for x in ["low budget", "small budget", "limited budget", "little money"]):
            return 3000
    ...
    def _detect_market(self, text: str):
        if any(x in text for x in [
            ...
            "many competitors", "bigger competitors", "similar businesses",
            "many businesses", "lots of competition"
        ]):
            return "competitive"

        if any(x in text for x in [
            "few competitors", "not many competitors", "low competition",
            "no competition", "new market"
        ]):
    ...
    def _detect_business_model(self, text: str):
        if any(x in text for x in ["restaurant", "food", "cafe", "bakery", "pizza", "shawarma"]):
            return "local_repeat_purchase_business"

        if any(x in text for x in ["software", "saas", "app", "platform", "ai tool", "automation"]):
            return "digital_product_or_platform"

        if any(x in text for x in ["agency", "consulting", "service", "freelance", "marketing"]):
            return "service_business"

        if any(x in text for x in ["clothing", "fashion", "brand", "sneaker", "beauty", "cosmetic"]):
            return "brand_driven_product_business"

        if any(x in text for x in ["building material", "construction", "cement", "tiles", "hardware"]):
            return "supply_and_distribution_business"

        if any(x in text for x in ["course", "tutor", "training", "education", "coaching"]):
    ...
    def _detect_customer_type(self, text: str):
        if any(x in text for x in ["contractor", "builders", "companies", "businesses", "b2b"]):
            return "business_customers"

        if any(x in text for x in ["students", "parents", "families"]):
            return "specialized_consumers"

        if any(x in text for x in ["customers", "people", "users", "buyers", "clients"]):
            return "general_consumers"

        return "unknown_customers"

    def _detect_channel(self, text: str):
        has_online = any(x in text for x in ["online", "website", "app", "social media", "ecommerce"])
        has_offline = any(x in text for x in ["offline", "physical", "store", "shop", "location"])
    ...
    def _detect_stage(self, text: str):
        if any(x in text for x in ["start", "starting", "open", "launch", "idea", "new business"]):
            return "early_stage"

        if any(x in text for x in ["grow", "scale", "more customers", "increase sales"]):
            return "growth_stage"

        if any(x in text for x in ["struggling", "losing money", "not selling", "low sales"]):
    ...
    def _score_trust_dependency(self, text, model):
        ...
        if any(x in text for x in ["expensive", "health", "money", "contractor", "businesses", "clients"]):
            score += 20
    ...
    def _score_repeat_purchase(self, text, model):
        ...
        if model in ["service_business", "supply_and_distribution_business", "knowledge_business"]:
            return "high"
    ...
    def _score_capital_intensity(self, text, model, channel):
        ...
        if model in ["local_repeat_purchase_business", "supply_and_distribution_business"]:
            score += 25

        if any(x in text for x in ["inventory", "rent", "equipment", "warehouse", "store"]):
            score += 20
    ...
    def _score_operational_complexity(self, text, model):
        ...
        if any(x in text for x in ["delivery", "inventory", "employees", "hiring", "supplier"]):
            score += 20
    ...
    def _score_price_sensitivity(self, text):
        if any(x in text for x in ["cheap", "low price", "price sensitive", "affordable", "budget customers"]):
            return "very_high"
    ...
    def _describe_business_nature(self, dna):
        ...
        if model == "local_repeat_purchase_business":
            return (
                "This business depends on repeat customers, local trust, consistency, and customer experience."
            )

        if model == "digital_product_or_platform":
            return (
                "This business depends on solving a clear problem, fast iteration, onboarding, and user retention."
            )

        if model == "supply_and_distribution_business":
            return (
                "This business depends on supplier reliability, stock availability, delivery speed, and trust."
            )

        if model == "brand_driven_product_business":
            return (
                "This business depends on perception, identity, trust, visuals, and customer loyalty."
            )

        if model == "service_business":
            return (
                "This business depends on trust, proof, communication, delivery quality, and client relationships."
            )

        return (
            "This business depends on clear positioning, customer trust, value delivery, and disciplined execution."
        )

    def _describe_market_nature(self, dna):
        if dna["competition_pressure"] in ["very_high", "high"]:
            return (
                "The market is pressure-heavy. Customers already have options, so unclear offers will be ignored."
            )

        return (
            "The market may offer room to enter, but demand still needs to be validated before scaling."
        )

    def _describe_customer_nature(self, dna):
        if dna["trust_dependency"] in ["very_high", "high"]:
            return (
                "Customers need proof before they act. Trust, reviews, clarity, and low-risk offers matter."
            )

        return (
            "Customers need a simple reason to care, understand, and try the offer."
        )

    def _describe_execution_reality(self, dna):
        if dna["capital_intensity"] in ["very_high", "high"]:
            return (
                "Execution requires cash discipline. Spending too early can create pressure before demand is proven."
            )

        return (
            "Execution can start lean. The priority is learning quickly before increasing spend."
        )
    ...
    def _strategic_direction(self, dna):
        ...
        if dna["scalability"] in ["very_high", "high"]:
            return (
                "Validate the core offer first, then scale through the strongest channel once customer behavior proves demand."
            )

        return (
            "Start controlled, prove demand, improve trust, and expand only after real customer signals appear."
        )


business_understanding_engine = BusinessUnderstandingEngine()