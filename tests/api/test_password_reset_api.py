...
def test_safe_route_stage_labels_preserve_generic_response(monkeypatch, caplog):
    ...
    assert "password_reset_stage=account_ineligible" in caplog.text
    ...
    assert "password_reset_stage=rate_limited" in caplog.text
    ...
    db.commit();db.close()
    assert client.post("/auth/password-reset/request",json={"email":"person@example.com"}).json()==expected
    assert "password_reset_stage=recent_token_limited" in caplog.text
